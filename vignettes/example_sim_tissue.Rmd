...
In this vignette, we illustrate how to apply the `TopKAT` method to analyze cell-level imaging data. We simulated this data using the `scSpatialSIM` package to provide a more realistic example of applying `TopKAT` to imaging data with cell-level resolution. 
...
Below, we show how to calculate the kernel matrices using `rips_similarity_matrix` using a maximum radius of $100$. We chose this maximum radius since the images are of dimension $100 \times 100$. There is no penalty for selecting a radius that exceeds the boundaries of the image. 

```{r compute kernel matrices}
# Compute the similarity matrix
simmat <- rips_similarity_matrix(data2.df, max.threshold = 100, print.progress = FALSE)
```
...
We first examine the sequence of p-values at each radius. The radius at which the minimum TopKAT p-value arose was $r=8.163265$. 
...
Finally, we can examine the connectivity of the cell types at this radius using connectivity matrices. These matrices enumerate how many connections there are between cells at $r= 8.2$. We visualize these matrices for the four samples given above. 
